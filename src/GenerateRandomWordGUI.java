...
public class GenerateRandomWordGUI extends JFrame {
	...
	private void setupGUI() {
		...
		// Target word input
		gbc.gridx = 2;
		inputPanel.add(new JLabel("Enter the depth level:"), gbc);
		...
		exploreButton.addActionListener(e -> generateRandomWordPath());
	...
	private void generateRandomWordPath() {
		String sourceWord = sourceWordField.getText().trim().toLowerCase();
		String depthLevel = depthLevelField.getText().trim().toLowerCase();

		if (sourceWord.isEmpty() || depthLevel.isEmpty()) {
		...
		currentPath = synonymGraph.generateWordAtDepth(sourceWord, depth);
		pathSynonyms = synonymGraph.getPathSynonyms(currentPath);

		if (currentPath == null) {
			analysisArea.setText("Source word not found");
			currentPath = null;
			pathSynonyms = null;
		} else {
			connectionLevel = currentPath.size() - 1;
	...
	private void calculateNodePositions() {
		nodePositions.clear();
		if (currentPath == null || pathSynonyms == null)
			return;
	...
	private void drawEnhancedGraph(Graphics g) {
		...
		for (String pathWord : currentPath) {
			...
			g2d.setStroke(new BasicStroke(1.0f, BasicStroke.CAP_BUTT, BasicStroke.JOIN_BEVEL, 0, new float[] { 3 }, 0));
		...
		for (Map.Entry<String, Point> entry : nodePositions.entrySet()) {
			...
			g2d.drawString(word, pos.x - (textWidth / 2), pos.y + (textHeight / 4));
		}
	}
	...
	public static void main(String[] args) {
		SwingUtilities.invokeLater(() -> {
			new GenerateRandomWordGUI().setVisible(true);
		});
	}
}